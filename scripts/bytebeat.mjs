...
drawGraphics(endTime) {
    ...
    for(let i = 0; i < bufferLen; ++i) {
        ...
        const isNaNCurY = [isNaN(curY[0]), isNaNCurY[1]];
    ...
    if (isFFT) {
        function fft(input) {
            ...
            const T = new Array(N).fill(0).map((_, k) => {
                const t = -2 * Math.PI * k / N;
                return Math.cos(t) - Math.sin(t) * 1j;
            });
            
            return even.map((e, i) => e + T[i] * odd[i]).concat(
                   even.map((e, i) => e - T[i] * odd[i]));
        }

        const fftData = fft(buffer.map(b => b.value[0])); // Assuming mono audio for simplicity
        console.log(fftData);
    }
}
