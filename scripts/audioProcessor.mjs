class audioProcessor extends AudioWorkletProcessor {
    constructor(...args) {
        ...
        this.lastByteValue = [null, null, null, null]; // Update to 4 channels
        this.lastFuncValue = [null, null, null, null]; // Update to 4 channels
        this.lastTime = -1;
        this.outValue = [0, 0, 0, 0]; // Update to 4 channels
    ...
    process(inputs, [chData]) {
        ...
        for (let i = 0; i < chDataLen; ++i) {
            ...
            if (this.lastTime !== currentTime) {
                ...
                try {
                    funcValue = this.isFuncbeat ? this.func(currentSample / this.sampleRate, this.sampleRate) :
                                this.isWavePot ? this.func(currentSample / 44100, 44100) :
                                this.isFuncBytebeat ? this.func(currentSample / this.sampleRate, this.sampleRate) :
                                this.isSignedFuncBytebeat ? this.func(currentSample / this.sampleRate, this.sampleRate) :
                                this.func(currentSample);
                } catch (err) {
                ...
                funcValue = Array.isArray(funcValue) ? funcValue : [funcValue, funcValue, funcValue, funcValue]; // Update to 4 channels
                let hasValue = false;
                let ch = 4; // Update to 4 channels
                while (ch--) {
                    try {
    ...
    resetValues() {
        this.audioSample = 0;
        this.lastByteValue = this.lastFuncValue = [null, null, null, null]; // Update to 4 channels
        this.lastTime = -1;
        this.outValue = [0, 0, 0, 0]; // Update to 4 channels
    }
