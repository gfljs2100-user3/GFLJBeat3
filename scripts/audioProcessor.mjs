class audioProcessor extends AudioWorkletProcessor {
	...
	setFunction(codeText) {
		const gfjs = {
			...
			"FM": function (a,b,c,d) { return Math.sin((a*2)+Math.sin(a*4)*4*Math.exp(-d%b/(c/4))+Math.sin(a*3)*3*Math.exp(-d%b/(c/4))+Math.sin(a)*8*Math.exp(-d%b/(c/4))+Math.sin(a*2)*3*Math.exp(-d%b/(c/4))*Math.sin(a*5)*2*Math.exp(-d%b/(c/4)))*Math.exp(-d%b/(c/4)) }
		}
