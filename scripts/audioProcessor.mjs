class audioProcessor extends AudioWorkletProcessor {
	...
	setFunction(codeText) {
		const gfjs = {
			...
			"FM": function (a,b,c,d) { return Math.sin((a*2)+Math.sin(a*2)*2*Math.exp(-d%b/(c/2))+Math.sin(a*3)*2*Math.exp(-d%b/(c/2))+Math.sin(a*2)*3*Math.exp(-d%b/(c/2))+Math.sin(a*2)*3*Math.exp(-d%b/(c/2))*Math.sin(a*2)*3*Math.exp(-d%b/(c/2)))*Math.exp(-d%b/(c/2)) }
		}
