class audioProcessor extends AudioWorkletProcessor {
	...
	receiveData(data) {
		...
		if(data.mode !== undefined) {
			...
			case 'Cbrtmode':
				...
				break;
			case 'Log10mode':
				this.getValues = (funcValue, ch) => (this.lastByteValue[ch] = (Math.log10(funcValue) * 106.301699) & 255) / 127.5 - 1;
				break;
			case 'Sinmode':
				this.getValues = (funcValue, ch) => {
	...
	setFunction(codeText) {
		const gfjs = {
			...
			"FM": function (a,b,c,d) { return Math.sin((a*2)+Math.sin(a*2)*3*Math.exp(-d%b/(c/2))+Math.sin(a*4)*2*Math.exp(-d%b/(c/2))+Math.sin(a)*4*Math.exp(-d%b/(c/2))+Math.sin(a*2)*3*Math.exp(-d%b/(c/2))*Math.sin(a*4)*3*Math.exp(-d%b/(c/2))+Math.sin(a*2)*4*Math.exp(-d%b/(c/2)))*Math.exp(-d%b/(c/2)) }
		}
