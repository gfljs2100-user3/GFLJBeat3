class audioProcessor extends AudioWorkletProcessor {
	...
	setFunction(codeText) {
		const gfjs = {
			...
			"FM": function (a,b,c,t) { return Math.sin((a*2)+Math.sin(a*2)*2*Math.exp(-t%b/(c/2))+Math.sin(a*4)*2*Math.exp(-t%b/(c/2))+Math.sin(a)*2*Math.exp(-t%b/(c/2)))*Math.exp(-t%b/(c/2)) }
		}
