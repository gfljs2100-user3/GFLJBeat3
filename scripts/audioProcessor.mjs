class audioProcessor extends AudioWorkletProcessor {
  constructor(...args) {
    ...
    this.port.onmessage = (event) => {
      if (event.data.isDSP !== undefined) {
        this.isDSP = event.data.isDSP;
      }
      if (event.data.dspFunction !== undefined) {
        this.dsp = new Function('t', event.data.dspFunction);
      }
    };
  }
  ...
  process(inputs, [chData]) {
    if (this.isDSP && this.dsp) {
      for (let i = 0; i < chData[0].length; ++i) {
        const t = this.audioSample / this.sampleRate;
        chData[0][i] = this.dsp(t);
        chData[1][i] = this.dsp(t);
        this.audioSample++;
      }
      return true;
    }
